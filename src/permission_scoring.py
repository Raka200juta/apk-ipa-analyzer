...
Centralized scoring logic used by the Flask app. Kept as a module under
`src` so imports are consistent after repository reorganization.
"""
...
def classify_permissions(permissions, suspicious_set=None):
    ...
    - suspicious_set: optional set of permission names treated as suspicious

    Returns:
    (safety_score:int, classification:str, reason:str, dangerous_list:list, risk_weight:float)
    """
    if not permissions:
